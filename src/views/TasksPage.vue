<template>
    <div class="flex h-screen bg-gray-100">
      <aside class="w-64 bg-gray-800 text-white">
        <div class="p-6">
          <h2 class="text-3xl font-bold">Task Management</h2>
        </div>
        <nav>
          <ul>
            <li>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
          ...
          <div v-if="!isLoading && !error" class="mt-6">
            ...
            <table class="min-w-full bg-white">
              <thead>
                <tr>
                  <th class="py-2 px-4 bg-gray-200">Task Name</th>
                  <th class="py-2 px-4 bg-gray-200">Assigned To</th>
              ...
              <tbody>
                <tr v-for="task in tasks" :key="task.id" class="text-center">
                  ...
                  <td class="py-2 px-4">
                    <button @click="editTask(task)" class="bg-blue-500 text-white py-1 px-2 rounded">Edit</button>
                  </td>
                </tr>
              </tbody>
            </table>
  ...
  export default {
    ...
    methods: {
      ...
      async editTask(task) {
        ...
        if (newTaskName) {
          try {
            await axios.put(`/api/tasks/${task.id}`, { name: newTaskName });
            this.fetchTasks();
            alert('Task updated successfully!');
          } catch (error) {
            console.error('Error updating task:', error);
            this.error = 'Failed to update the task';
          }
        }
      },
    },
