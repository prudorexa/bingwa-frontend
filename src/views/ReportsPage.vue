    ...
    <div class="flex h-screen bg-gray-100">
      ...
      <div class="flex-1 p-6">
        <div class="bg-white shadow-md rounded-lg p-6">
          ...
          <p class="text-gray-600">View and manage reports here.</p>
          ...
          <div v-if="!isLoading && !error" class="mt-6">
            <h2 class="text-xl font-semibold mb-4">Report List</h2>
            <table class="min-w-full bg-white">
              <thead>
                <tr>
                  <th class="py-2 px-4 bg-gray-200">Report Name</th>
                  <th class="py-2 px-4 bg-gray-200">Date</th>
                  <th class="py-2 px-4 bg-gray-200">Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="report in reports" :key="report.id" class="text-center">
                  <td class="py-2 px-4">{{ report.name }}</td>
                  <td class="py-2 px-4">{{ report.date }}</td>
                  <td class="py-2 px-4">
                    <button @click="viewReport(report)" class="bg-blue-500 text-white py-1 px-2 rounded">View</button>
                  </td>
                </tr>
              </tbody>
            </table>
